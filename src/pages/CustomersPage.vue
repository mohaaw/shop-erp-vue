<template>
  <div class="customers-page">
    ...
    <div v-if="customerStore.isLoading" class="text-center p-5">
      <div class="loader"></div> Loading customers...
    </div>
    <div v-else-if="customerStore.error" class="card"><div class="card-body form-alert alert-danger">{{ customerStore.error }}</div></div>
      ...
      <div class="table-responsive">
        <table class="table table-hover">
          ...
          <tr v-for="customer in customerStore.filteredCustomers" :key="customer.idInternal">
            ...
            <td>
              <button @click="openCustomerModal(customer.idInternal)" class="btn btn-sm btn-info mr-1" title="Edit Customer">
    ...
    <div v-else class="card">
      <div class="card-body text-center">
        <p v-if="customerStore.searchTerm && customerStore.searchTerm.length > 0">No customers match your search criteria.</p>
        <p v-else>No customers found.
          <a href="#" @click.prevent="openCustomerModal(null)">Add your first customer!</a>
        </p>
      </div>
    </div>
...
const settingsStore = useSettingsStore(); // For currency symbol
...
onMounted(() => {
  ...
  // Reset search term if desired when navigating to this page
  // customerStore.setSearchTerm('');
});

// Optional: Clear search term when leaving the page
// onUnmounted(() => {
//   customerStore.setSearchTerm('');
// });
...
const confirmDelete = (customer) => {
  if (window.confirm(`Are you sure you want to delete customer "${customer.name}"? This may affect sales records associated with them.`)) {
...
.loader { margin-bottom: 0.5rem; } /* Adjust loader margin if needed */
/* Re-using alert style from ProductForm */
